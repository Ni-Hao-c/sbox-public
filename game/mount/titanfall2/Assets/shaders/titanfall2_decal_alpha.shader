HEADER
{
	Description = "Titanfall 2 straight-alpha mesh decal shader";
}
...
PS
{
	...
	CreateInputTexture2D( Albedo, Srgb, 8, "None", "_color", ",0/,0/0", Default4( 0.50, 0.50, 0.50, 0.00 ) );
	...
	float g_flDecalOpacity < Attribute( "g_flDecalOpacity" ); Default( 1.0 ); >;

	float3 DecodeTitanfall2DecalNormal( PixelInput i )
	{
		...
		float3 tangentNormal = Tex2DS( g_tNormal, g_sAniso, uv ).xyz * 2.0 - 1.0;
	...
	Material BuildTitanfall2DecalMaterial( PixelInput i, out float opacity )
	{
		...
		opacity = albedo.a * opacityMap * i.vVertexColor.a * g_flDecalOpacity * g_flT2MaterialOpacity;

		Material m = Material::Init();
		m.Albedo = T2ApplyDetail( albedo.rgb, uv ) * g_vT2AlbedoTint.rgb * i.vVertexColor.rgb;
		m.Normal = DecodeTitanfall2DecalNormal( i );
		m.Roughness = T2Roughness( gloss );
		m.Metalness = T2Metalness( specular );
		m.AmbientOcclusion = ao;
		m.TintMask = 1.0;
		m.Opacity = opacity;
		m.Emission = emissive * g_vT2EmissiveTint.rgb * i.vVertexColor.rgb * g_flT2EmissiveStrength;
	...
	{
		float opacity;
		Material m = BuildTitanfall2DecalMaterial( i, opacity );
		clip( opacity - 0.0001 );
		float4 shade = ShadingModelStandard::Shade( i, m );
		return float4( shade.rgb, opacity );
	}
}
